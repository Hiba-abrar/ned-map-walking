...
map<string, int> locationToNode = {
    ...
    {"Survey Lab", 7},
    {"NPO Circuit", 17},
    {"NED University Admin Block", 1},
    {"NED University Library", 2},
    {"Urban Infrastructure Engineering Department", 24},
    {"Civil Engineering Class Rooms", 24},
    {"Fire Lab", 23},
    {"CSIT Labs", 15},
    {"CSIT Department Entrance", 15},
    {"CSIT Offices", 16},
    {"Mosque", 2},
    {"Basketball Court", 20},
    {"Tennis Court", 21},
    {"Convocation Ground", 25},
    {"Civil AV Hall", 8},
...
vector<NavigationStep> calculate_road_route(const string& start_name, const string& end_name) {
    ...
    for (size_t i = 0; i < path.size(); i++) {
        ...
        if (i == 0) {
            ...
            step.instruction = "Start walking from " + start_name;
        }
        else if (i == path.size() - 1) {
            step.maneuver_type = "arrive";
            step.direction = "straight";
            step.is_turn_point = false;
            step.instruction = "You have arrived at " + end_name;
        }
        else {
            ...
            if (step.is_turn_point) {
                ...
                if (distance_to_next > 50) {
                    step.instruction = "Continue straight for " + to_string(int(distance_to_next)) + " meters";
                } else {
